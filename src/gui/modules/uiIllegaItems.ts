...
import { IllegalItemsB } from "../../penrose/PlayerPlaceBlockAfterEvent/illegalitems/illegalitems_b";
...
export function uiILLEGALITEMS(illegalitemsResult: ModalFormResponse, player: Player) {
    ...
    if (uniqueId !== player.name) {
        return sendMsgToPlayer(player, `§f§4[§6Paradox§4]§f 管理者しか実行できません to configure Illegal Items`);
    }
    ...
    if (IllegalItemsBToggle === true && illegalItemsBBoolean === false) {
        ...
        sendMsg("@a[tag=paradoxOpped]", `§f§4[§6Paradox§4]§f ${player.name}§f 以下の機能が有効です！＝＞ §6IllegalItemsB§f!`);
        IllegalItemsB();
    }
    if (IllegalItemsBToggle === false && illegalItemsBBoolean === true) {
        ...
        sendMsg("@a[tag=paradoxOpped]", `§f§4[§6Paradox§4]§f ${player.name}§f 以下の機能が無効です！＝＞ §4IllegalItemsB§f!`);
    }
    if (IllegalItemsCToggle === true && illegalItemsCBoolean === false) {
        ...
        sendMsg("@a[tag=paradoxOpped]", `§f§4[§6Paradox§4]§f ${player.name}§f 以下の機能が有効です！＝＞ §6IllegalItemsC§f!`);
        IllegalItemsC();
    }
    if (IllegalItemsCToggle === false && illegalItemsABoolean === true) {
        ...
        sendMsg("@a[tag=paradoxOpped]", `§f§4[§6Paradox§4]§f ${player.name}§f 以下の機能が無効です！＝＞ §4IllegalItemsC§f!`);
    }
    if (IllegalEnchanmentsToggle === true && illegalEnchantmentBoolean === false) {
        ...
        sendMsg("@a[tag=paradoxOpped]", `§f§4[§6Paradox§4]§f ${player.name}§f 以下の機能が有効です！＝＞ §6IllegalEnchantments§f!`);
    }
    if (IllegalEnchanmentsToggle === false && illegalEnchantmentBoolean === true) {
        ...
        sendMsg("@a[tag=paradoxOpped]", `§f§4[§6Paradox§4]§f ${player.name}§f 以下の機能が無効です！＝＞ §4IllegalEnchantments§f!`);
    }
