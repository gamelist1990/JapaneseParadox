...
export function stackban(message: ChatSendAfterEvent, args: string[]) {
    ...
    if (stackBanBoolean === false) {
        ...
        sendMsg("@a[tag=paradoxOpped]", `§f§4[§6Paradox§4]§f §7${player.name}§f 有効にしました＝＞ §6StackBans§f!`);
    } else if (stackBanBoolean === true) {
        // Deny
        dynamicPropertyRegistry.set("stackban_b", false);
        world.setDynamicProperty("stackban_b", false);
        sendMsg("@a[tag=paradoxOpped]", `§f§4[§6Paradox§4]§f §7${player.name}§f 無効にしました＝＞ §4StackBans§f!`);
    }
}
