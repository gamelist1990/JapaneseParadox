...
export function invalidsprintA(message: ChatSendAfterEvent, args: string[]) {
    ...
    if (invalidSprintABoolean === false) {
        ...
        sendMsg("@a[tag=paradoxOpped]", `§f§4[§6Paradox§4]§f §7${player.name}§f 有効にしました＝＞ §6InvalidSprintA§f!`);
        InvalidSprintA();
        ...
        sendMsg("@a[tag=paradoxOpped]", `§f§4[§6Paradox§4]§f §7${player.name}§f 無効にしました＝＞ §4InvalidSprintA§f!`);
    }
}
