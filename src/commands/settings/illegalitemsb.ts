...
export function illegalitemsB(message: ChatSendAfterEvent, args: string[]) {
    ...
    if (illegalItemsBBoolean === false) {
        ...
        sendMsg("@a[tag=paradoxOpped]", `§f§4[§6Paradox§4]§f §7${player.name}§f 有効にしました＝＞ §6IllegalItemsB§f!`);
        IllegalItemsB();
        ...
        sendMsg("@a[tag=paradoxOpped]", `§f§4[§6Paradox§4]§f §7${player.name}§f 無効にしました＝＞ §4IllegalItemsB§f!`);
    }
}
