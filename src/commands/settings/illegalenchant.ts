...
function illegalEnchantHelp(player: Player, prefix: string, illegalEnchantmentBoolean: boolean, setting: boolean): void {
    ...
    const moduleStatus: string = illegalEnchantmentBoolean ? "§6[§a有効§6]§f" : "§6[§4無効§6]§f";

    // 選手にヘルプ情報を表示する
    sendMsgToPlayer(player, [
        `§n§o§4[§6コマンド§4]§f: イリーガルエンチャント`,
        `§4[§6Status§4]§f: ${commandStatus}`,
        `§4[§6Module§4]§f: ${moduleStatus}`,
        `§4[§6Usage§4]§f: ${prefix}illegalenchant [options]`,
        `§4[§6Description§4]§f：不正なエンチャントを持つアイテムのチェックを切り替える。`,
        `§4[§6オプション§4]§f：`,
        `    -h, --help`,
        `       §4[§7このヘルプメッセージを表示する§4]§f`,
        `    -s, --status`,
        `       §4[§7IllegalEnchantmentsモジュールの現在の状態を表示する§4]§f`,
        `    -e, --enable`,
        `       §4[§7イリーガルエンチャントモジュールをBooleanにする§4]§f`,
        `    -d, --disable`,
        `       §4[§7イリーガルエンチャントモジュールを無効にする§4]§f`,
    ]);
}
    ...
    for (let i = 0; i < length; i++) {
        ...
        switch (additionalArg) {
            ...
            case "--status":
                // ハンドル状態フラグ
                validFlagFound = true;
                sendMsgToPlayer(player, `§f§4[§6Paradox§4]§f IllegalEnchantments module is currently ${configuration.modules.illegalEnchantment.enabled ? "有効" : "無効"}`);
                break;
            case "-e":
            case "--enable":
                // ハンドルイネーブルフラグ
                ...
                if (configuration.modules.illegalEnchantment.enabled) {
                    sendMsgToPlayer(player, `§f§4[§6Paradox§4]§f IllegalEnchantments モジュールは既にBooleanになっている。`);
                } else {
                    configuration.modules.illegalEnchantment.enabled = true;
                    dynamicPropertyRegistry.setProperty(undefined, "paradoxConfig", configuration);
                    sendMsg("@a[tag=paradoxOpped]", `§f§4[§6Paradox§4]§f §7${player.name}§f 以下の機能が有効です=> §6IllegalEnchantments§f!`);
                }
                break;
            case "-d":
            case "--disable":
                ...
                validFlagFound = true;
                if (!configuration.modules.illegalEnchantment.enabled) {
                    sendMsgToPlayer(player, `§f§4[§6Paradox§4]§f IllegalEnchantments モジュールは既に無効化されている。`);
                } else {
                    configuration.modules.illegalEnchantment.enabled = false;
                    dynamicPropertyRegistry.setProperty(undefined, "paradoxConfig", configuration);
                    sendMsg("@a[tag=paradoxOpped]", `§f§4[§6Paradox§4]§f §7${player.name}§f は無効 §4IllegalEnchantments§f!`);
                }
                break;
        }
    }
