...
export function badpackets2(message: ChatSendAfterEvent, args: string[]) {
    ...
    if (badPackets2Boolean === false) {
        ...
        sendMsg("@a[tag=paradoxOpped]", `§f§4[§6Paradox§4]§f §7${player.name}§f 有効にしました＝＞ §6Badpackets2§f!`);
        BadPackets2();
        ...
        sendMsg("@a[tag=paradoxOpped]", `§f§4[§6Paradox§4]§f §7${player.name}§f 無効にしました＝＞ §4Badpackets2§f!`);
    }
}
