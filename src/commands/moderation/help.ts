...
export function help(message: ChatSendAfterEvent) {
    ...
    return sendMsgToPlayer(player, [
        ...
        `§6${prefix}speeda§f - ${config.customcommands.speeda ? `プレイヤーがスピードハックをしているかどうかをチェックする。` : textDisabled}`,
        ...
        `§6${prefix}rank§f - ${config.customcommands.rank ? `タグを付けれます` : textDisabled}`,
