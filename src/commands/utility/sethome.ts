...
function setHomeHelp(player: Player, prefix: string, configuration: ConfigInterface) {
    ...
    return sendMsgToPlayer(player, [
        `\n§o§4[§6コマンド§4]§f: sethome`,
        `§4[§6ステータス§4]§f: ${commandStatus}`,
        `§4[§6使用法§4]§f: sethome [optional]`,
        `§4[§6説明§4]§f: Saves home location based on current coordinates. Up to ${configuration.modules.setHome.max} total.`,
        `§4[§6Options§4]§f:`,
        `    -h, --help`,
...
export function sethome(message: ChatSendAfterEvent, args: string[]) {
    ...
    if (!message) {
        return console.warn(`${new Date()} | ` + "エラー: ${message} が定義されていません。渡すのを忘れましたか? ./commands/utility/sethome.js:26)");
    }
        ...
        switch (additionalArg) {
            ...
            case "--enable":
                ...
                if (configuration.customcommands.sethome) {
                    ...
                    sendMsg("@a[tag=paradoxOpped]", `§f§4[§6Paradox§4]§f §7${player.name}§f 有効になりました: §6SetHome§f!`);
                }
                break;
            case "-d":
            case "--disable":
                ...
                if (!configuration.customcommands.sethome) {
                    ...
                    dynamicPropertyRegistry.setProperty(undefined, "paradoxConfig", configuration);
                    sendMsg("@a[tag=paradoxOpped]", `§f§4[§6Paradox§4]§f §7${player.name}§f 無効にしました: §4SetHome§f!`);
                }
                break;
            case "-m":
            case "--max": {
                ...
                if (isNaN(numberConvert)) {
                    return sendMsgToPlayer(
                        player,
                        `§f§4[§6Paradox§4]§f Invalid option. Use ${prefix}sethome --help を参照してください。
`
                    );
                }
...
`
        ...
        sendMsgToPlayer(
            player,
            `§f§4[§6Paradox§4]§f 無効なコマンドです。使用するには=> ${prefix}sethome --help を参照してください。
`
        );
    }
}
