...
function clearLag(id) {
    ...
    if (timeLeft <= 0) {
        ...
        sendMsg("@a", `§f§4[§6Paradox§4]§f サーバー最適化完了！`);
        ...
        if (timeLeft === 60000) {
            sendMsg("@a", `§f§4[§6Paradox§4]§f サーバー最適化まで後一分`);
        }
        else {
            const secondsLeft = Math.ceil(timeLeft / 1000);
            const countdownMsg = secondsLeft === 1 ? "秒" : "秒";
            sendMsg("@a", `§f§4[§6Paradox§4]§f サーバー最適化まで ${secondsLeft} ${countdownMsg}!`);
        }
        warned = true;
    }
