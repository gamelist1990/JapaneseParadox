...
export function uiDEOP(opResult, onlineList, player) {
    ...
    if (memberHash !== undefined && memberHash === memberEncode) {
        ...
        if (player.name !== member.name) {
            sendMsg("@a[tag=paradoxOpped]", `§f§4[§6Paradox§4]§f ${member.name}§f 管理者ではなくなりました`);
        }
        sendMsgToPlayer(member, `§f§4[§6Paradox§4]§f あなたのOPステータスは取り消されました!`);
        return paradoxui(player);
    }
    sendMsgToPlayer(player, `§f§4[§6Paradox§4]§f ${member.name}Op権限を持っていません`);
    return paradoxui(player);
}
