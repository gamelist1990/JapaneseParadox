...
async function handleUIDespawner(despawnerResult, player) {
    ...
    if (DespawnAllToggle === false) {
        ...
        if (counter > 0) {
            sendMsgToPlayer(player, ` §6|§f §4[§f${requestedEntity}§4]§f§4x${counter}§f匹`);
        }
        else {
            sendMsgToPlayer(player, `§f§4[§6Paradox§4]§fモブが見つかりません`);
        }
    }
    if (DespawnAllToggle === true) {
        ...
        for (const entity in entityCount) {
            if (entityCount.hasOwnProperty(entity)) {
                ...
                if (count > 0) {
                    entityMessage += ` §6|§f §4[§f${entity}§4]§f §4x${count}§f匹\n`;
                    totalCounter += count;
                }
            }
        }
        if (totalCounter > 0) {
            ...
            sendMsgToPlayer(player, `§f§4[§6Paradox§4]§f 見つかりませんでした`);
        }
    }
    return paradoxui;
}
