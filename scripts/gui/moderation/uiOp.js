    ...
    if ("formValues" in opResult) {
        ...
        if (onlineList.length > 0) {
            ...
            if (config.encryption.password !== value) {
                // Incorrect password
                return sendMsgToPlayer(player, `§f§4[§6Paradox§4]§f パスワードが違います。`);
            }
        }
        if (targetPlayer) {
            ...
            if (targetHash === undefined) {
                ...
                sendMsgToPlayer(player, `§f§4[§6Paradox§4]§f ${targetPlayer.name}管理者権限を取得しました`);
                sendMsgToPlayer(targetPlayer, `§f§4[§6Paradox§4]§f 管理者権限を取得しました`);
                sendMsg("@a[tag=paradoxOpped]", `§f§4[§6Paradox§4]§f ${targetPlayer.name}§f 新しい管理者が来たよ！`);
                targetPlayer.addTag("paradoxOpped");
                return paradoxui(player);
            }
            else {
                sendMsgToPlayer(player, `§f§4[§6Paradox§4]§f ${targetPlayer.name} 既に管理者権限を持っています`);
                return paradoxui(player);
            }
        }
        else {
            sendMsgToPlayer(player, `§f§4[§6Paradox§4]§f 存在しない又はオフラインです ${targetPlayer.name}.`);
            return paradoxui(player);
        }
    }
        ...
        if (opResult.selection === 0) {
            ...
            sendMsgToPlayer(player, `§f§4[§6Paradox§4]§f 管理者権限が付与されました`);
