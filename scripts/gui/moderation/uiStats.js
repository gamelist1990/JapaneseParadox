...
export function uiSTATS(statsResult, onlineList, player) {
    ...
    switch (true) {
        case member.hasTag("freeze"):
            reportBody.push(`§f§4[§6Paradox§4]§f §6${member.name}§f は行動を制限されています　検知内容＝＞ ${member.hasTag("freezeAura") ? "AntiKillAura" : member.hasTag("freezeNukerA") ? "AntiNukerA" : member.hasTag("freezeScaffoldA") ? "AntiScaffoldA" : "Staff"}`);
            break;
