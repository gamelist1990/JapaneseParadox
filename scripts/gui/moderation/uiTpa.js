...
export function uiTPA(tpaResult, onlineList, player) {
    ...
    if (toggleTargetTo === true && toggleToTarget === true) {
        sendMsgToPlayer(player, `§f§4[§6Paradox§4]§f 両方のオプションを有効にすることはできません。`);
        return paradoxui(player);
    }
    //check to make sure the player 以下の機能が有効です！＝＞ at least one option.
    if (toggleTargetTo === false && toggleToTarget === false) {
        sendMsgToPlayer(player, `§f§4[§6Paradox§4]§f 1つのオプションを有効にする必要があります。`);
        return paradoxui(player);
    }
