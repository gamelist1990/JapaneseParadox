...
async function handleUIFreeze(freezeResult, onlineList, player) {
    ...
    if (boolean) {
        ...
        sendMsgToPlayer(member, `§f§4[§6Paradox§4]§fフリーズが解除されました`);
        sendMsg(`@a[tag=paradoxOpped]`, `${member.name}§f フリーズを解除しました`);
        return;
    }
    if (!boolean) {
        // Blindness
        ...
        sendMsgToPlayer(member, `§f§4[§6Paradox§4]§f フリーズしました！！`);
