 ...
 */
export function uiPUNISH(punishResult, onlineList, player) {
...
async function handleUIPunish(punishResult, onlineList, player) {
    ...
    const inventoryContainer = member.getComponent("minecraft:inventory");
    ...
    sendMsgToPlayer(member, `§f§4[§6Paradox§4]§f 消去しました`);
    // Use try/catch in case nobody has tag 'notify' as this will report 'no target selector'
    sendMsg("@a[tag=paradoxOpped]", `§f§4[§6Paradox§4]§f ${player.name}§f が${member.name}のインベントリを消去しました§f`);
