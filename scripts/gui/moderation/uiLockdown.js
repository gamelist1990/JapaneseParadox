 ...
 */
export function uiLOCKDOWN(lockdownResult, player) {
...
async function handleUILockdown(lockdownResult, player) {
    ...
    if (LockdownToggle === true) {
        ...
        // Shutting it down
        sendMsg("@a[tag=paradoxOpped]", `§f§4[§6Paradox§4]§f メンテナンス中です`);
    ...
    if (LockdownToggle === false) {
        ...
        sendMsg("@a[tag=paradoxOpped]", `§f§4[§6Paradox§4]§f メンテナンスが解除されました`);
    }
    return paradoxui;
}
