...
export function commandHandler(player, message) {
    ...
    if (config.debug)
        console.warn(`${new Date()} | "${player.name}" が以下のコマンドを実行しました: ${config.customcommands.prefix}${commandName} ${args.join(" ")}`);
...
export function handleCommandAfterSend(chatSendAfterEvent) {
    // Logic for handling the command after the message is sent
    if (!chatSendAfterEvent.message.startsWith(config.customcommands.prefix))
        return;
    // Do not broadcast any message to any targets
