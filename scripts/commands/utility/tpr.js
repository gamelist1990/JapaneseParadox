...
function teleportRequestHandler({ sender, message }) {
    const player = sender;
    ...
    if (!target) {
        sendMsgToPlayer(player, "§f§4[§6Paradox§4]§f 指定したプレイヤーが存在しない又はオフラインです");
        return;
    }
...
function teleportRequestApprovalHandler(object) {
...
export function TeleportRequestHandler({ sender, message }, args) {
