...
async function handleFreeze(message, args) {
    ...
    if (uniqueId2 === member.name) {
        return sendMsgToPlayer(player, `§f§4[§6Paradox§4]§f 管理者には適応できません`);
    }
    ...
    if (boolean) {
        ...
        sendMsg(`@a[tag=paradoxOpped]`, `${member.name}§fのフリーズが解かれました！`);
        return;
    }
    if (!boolean) {
        // Blindness
        ...
        sendMsgToPlayer(member, `§f§4[§6Paradox§4]§f荒らし疑惑があるためフリーズされました`);
        sendMsg(`@a[tag=paradoxOpped]`, `${member.name}§fがフリーズされました！！`);
        return;
    }
}
