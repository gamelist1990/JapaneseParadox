import { GameMode, Vector, world } from "@minecraft/server";
 ...
 */
export function flag(player, check, checkType, hackType, item, stack, debugName, debug, shouldTP) {
 ...
 */
export function resetTag(member) {
 ...
 */
export function encryptString(str, salt) {
 ...
 */
export function decryptString(str, salt) {
 ...
 */
export function setTimer(player, spawn = false) {
    let timer = 0;
    if (spawn === true) {
 ...
 */
export function isTimerExpired(player) {
 ...
 */
export const sendMsg = async (targets, message) => {
    const targetsArray = Array.isArray(targets) ? targets : [targets];
    const isArray = Array.isArray(message);
    const modifiedMessage = isArray ? message.map((msg) => msg.replace(/§f/g, "§f§o")) : message.replace(/§f/g, "§f§o");
 ...
 * @param message The message to send. This can be a string or an array of strings.
 */
export const sendMsgToPlayer = async (target, message) => {
    const isArray = Array.isArray(message);
    let modifiedMessage;
    if (isArray) {
        modifiedMessage = message.map((msg) => msg.replace(/§f/g, "§f§o")).join("\n");
    }
    else {
        modifiedMessage = message.replace(/§f/g, "§f§o");
    }
    target.runCommandAsync(`tellraw @s {"rawtext":[{"text":${JSON.stringify("\n" + modifiedMessage)}}]}`);
};
export const chatChannels = {};
export const playerChannelMap = {};
/**
 ...
 */
export function createChatChannel(channelName, password, owner) {
 ...
 */
export function inviteToChatChannel(playerName, channelName) {
 ...
 */
export function switchChatChannel(playerName, channelName, password) {
 ...
 */
export function getPlayerChannel(playerName) {
 ...
 */
export function handOverChannelOwnership(channelName, currentOwner, newOwnerName) {
    const channel = chatChannels[channelName];
    if (channel) {
 ...
 */
export function deleteChatChannel(channelName, password) {
    const channel = chatChannels[channelName];
    if (channel) {
        if (channel.password) {
