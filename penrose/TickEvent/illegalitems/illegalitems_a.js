...
const enchantmentPresenceMap = new Map();
// Create a map of player objects and their enchantment data
const enchantmentDataMap = new Map();
...
const itemStackDataMap = new Map();
// Create a map of player objects and their unverified ItemStack
const unverifiedItemMap = new Map();
// Create a map to store the previous empty slots count for each player
const previousEmptySlotsCount = new Map();
function rip(player, inventory_item, enchData, lore = false) {
    let reason;
    if (enchData) {
        ...
        if (currentEmptySlotsCount !== previousEmptySlotsCount.get(player.id)) {
            ...
            for (let i = 0; i < playerContainerSize; i++) {
                ...
                if (antiShulkerBoolean && itemStackId.includes("shulker")) {
                    ...
                    sendMsg("@a[tag=notify]", `§f§4[§6Paradox§4]§f 消去アイテム＝＞ ${itemStackId.replace("minecraft:", "")} 所持者＝＞ ${player.name}.`);
                    sendMsgToPlayer(player, `§f§4[§6Paradox§4]§f シュルカーボックスの使用は禁止されている！`);
                    continue;
                }
                // Illegal Stacks
                ...
                if (stackBanBoolean && currentStack > maxStack) {
                    playerContainer.setItem(i);
                    sendMsg("@a[tag=notify]", `§f§4[§6Paradox§4]§f 消去アイテム＝＞ ${itemStackId.replace("minecraft:", "")} x ${currentStack} 所持者＝＞ ${player.name}.`);
                    sendMsgToPlayer(player, `§f§4[§6Paradox§4]§f 不正なスタックは認められない！`);
                ...
                if (illegalitems.has(itemStackId)) {
                    playerContainer.setItem(i);
                    sendMsg("@a[tag=notify]", `§f§4[§6Paradox§4]§f 消去アイテム＝＞ ${itemStackId.replace("minecraft:", "")} 所持者＝＞ ${player.name}.`);
                    sendMsgToPlayer(player, `§f§4[§6Paradox§4]§f 違法アイテムの使用は禁止されている！`);
                    rip(player, playerItemStack);
                    break;
                }
                // Illegal Lores
                if (illegalLoresBoolean && !config.modules.illegalLores.exclude.includes(String(playerItemStack.getLore()))) {
                    playerContainer.setItem(i);
                    sendMsg("@a[tag=notify]", `§f§4[§6Paradox§4]§f 消去アイテム ${itemStackId.replace("minecraft:", "")} 所持者＝＞ ${player.name}.`);
                    sendMsgToPlayer(player, `§f§4[§6Paradox§4]§f 違法な名前を持つアイテムは使用できない！`);
                    rip(player, playerItemStack, null, true);
                    break;
                }
                // Illegal Enchantments
                if (illegalEnchantmentBoolean) {
                    const enchantmentComponent = playerItemStack.getComponent("minecraft:enchantments");
                    ...
                    while (!iteratorResult.done) {
                        const enchantment = iteratorResult.value;
                ...
                if (salvageBoolean) {
                    ...
                    if (!uniqueItems.includes(itemStackId)) {
                        ...
                        if (!verifiedItemName) {
                            unverifiedItemMap.set(player.id, new Map());
                        }
            ...
            if (illegalEnchantmentBoolean) {
                ...
                for (const [enchantment, present] of enchantmentPresenceMap.get(player.id)) {
                    if (present) {
                        ...
                        const newEnchantmentComponent = newItemStack.getComponent("minecraft:enchantments");
                        ...
                        if (currentLevel > maxLevel || currentLevel < 0 || !canAddEnchantBoolean) {
                            ...
                            sendMsg("@a[tag=notify]", `§f§4[§6Paradox§4]§f 消去アイテム＝＞ ${itemStackId.typeId.replace("minecraft:", "")} 所持者＝＞ ${player.name}.`);
                            sendMsgToPlayer(player, `§f§4[§6Paradox§4]§f 違法なエンチャントが施されたアイテムは使用できない！`);
            ...
            if (salvageBoolean) {
                ...
                for (const [slot, itemStackData] of unverifiedItemMap.get(player.id)) {
                    ...
                    const originalEnchantmentComponent = itemStackData.getComponent("minecraft:enchantments");
                    ...
                    // Get the new enchantment component from the new ItemStack
                    const newEnchantmentComponent = applyCustomProperties.getComponent("minecraft:enchantments");
                    // Get the new enchantment data from the new ItemStack component
                    const newEnchantmentData = newEnchantmentComponent.enchantments;
