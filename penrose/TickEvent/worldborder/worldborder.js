...
function worldborder(id) {
    ...
    for (const player of players) {
        ...
        let overworldSize = worldBorderOverworldNumber;
        let netherSize = worldBorderNetherNumber;
        let endSize = worldBorderEndNumber;
        ...
        if (player.dimension.id === "minecraft:overworld") {
            ...
            if (x > overworldSize || x < -overworldSize || z > overworldSize || z < -overworldSize) {
                sendMsgToPlayer(player, `§f§4[§6Paradox§4]§f あなたはワールドボーダーに到着しました`);
