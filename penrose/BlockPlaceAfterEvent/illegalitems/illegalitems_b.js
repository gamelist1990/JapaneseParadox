...
function rip(player, inventory_item, enchData, block, nested = false) {
    let reason;
    if (block) {
...
async function illegalitemsb(object) {
    ...
    const blockInventory = block.getComponent("minecraft:inventory");
        ...
        if (blockContainer) {
            ...
            for (let i = 0; i < blockContainerSize; i++) {
                ...
                if (stackBanBoolean && currentStack > maxStack) {
                    ...
                    sendMsgToPlayer(player, `§f§4[§6Paradox§4]§f 不正なスタックです`);
                ...
                if (itemStackId in illegalitems) {
                    ...
                    sendMsgToPlayer(player, `§f§4[§6Paradox§4]§f 違法アイテムの使用は禁止されています！`);
                    rip(player, blockItemStack, null, block);
                    isFlagged = true;
                    break;
                }
                ...
                if (illegalLoresBoolean && !config.modules.illegalLores.exclude.includes(String(blockItemStack.getLore()))) {
                    ...
                    sendMsgToPlayer(player, `§f§4[§6Paradox§4]§f違法な名前を持つアイテムの使用は禁止です！！`);
                    rip(player, blockItemStack, null, block);
                    isFlagged = true;
                    break;
                }
                ...
                if (illegalEnchantmentBoolean) {
                    const enchantmentComponent = blockItemStack.getComponent("minecraft:enchantments");
